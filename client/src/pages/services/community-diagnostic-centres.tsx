...
import { MessageSquare, Building, Users, Clock, CheckCircle, Timer, Shield, HeartHandshake, PoundSterling, Eye } from 'lucide-react';
...
import { communityDiagnosticCentresApi } from '@/lib/strapi/api/community-diagnostic-centres';
import type { CommunityDiagnosticPage, StrapiServiceFeature } from '@/lib/strapi/types/community-diagnostic-centres';
import type { StrapiGlobalSettings } from '@/lib/strapi/types/global-settings';

export default function CommunityDiagnosticCentres() {
  ...
  const [pageData, setPageData] = useState<CommunityDiagnosticPage | null>(null);
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center">
        <SEOHead {...SEO_DATA.communityDiagnostic} />
        <div className="text-center max-w-md mx-auto px-6">
          ...
          <p className="text-gray-600 mb-6">
            We're having trouble loading the Community Diagnostic Centres page. Please try refreshing or contact us if the issue persists.
          </p>
          <Button 
  ...
  const heroContent = {
    title: pageData?.heroTitle || "Community Diagnostic Centres",
    subtitle: pageData?.heroSubtitle || "Comprehensive and efficient diagnostic facilities for improved patient access and effective clinical pathways.",
    button1: {
      ...
      icon: <Eye className="h-5 w-5 group-hover:pulse transition-transform duration-300" />,
      analytics: "VIEW Equipment - Community Diagnostic Centres Hero"
    },
    button2: {
      ...
      analytics: "CONTACT Us - Community Diagnostic Centres Hero"
    }
  };

  // Service details - CMS data prioritized, static fallback
  const serviceDetails = {
    title: pageData?.serviceDetailsTitle || "Accessible Community Healthcare",
    description: pageData?.serviceDetailsDescription || "Our Community Diagnostic Centres provide convenient, local access to advanced imaging services, reducing wait times and improving patient outcomes through strategic community placement.",
    image: {
      src: pageData?.serviceImage || "/images/services/community-diagnostic-centre-hero.jpg",
      alt: pageData?.serviceImageAlt || "Modern community diagnostic centre exterior with accessible patient entrance"
    },
    features: pageData?.features && pageData.features.length > 0 ? pageData.features.map((feature: StrapiServiceFeature, index: number) => {
      const staticIcons = [
        <Building className="h-6 w-6 text-compleo-teal" />,
        <CheckCircle className="h-6 w-6 text-compleo-teal" />,
        <Clock className="h-6 w-6 text-compleo-teal" />,
        <Users className="h-6 w-6 text-compleo-teal" />
      ];
      return {
        icon: staticIcons[index % staticIcons.length] || <Building className="h-6 w-6 text-compleo-teal" />,
        title: feature.title,
        description: feature.description
      };
    }) : [
      {
        icon: <Building className="h-6 w-6 text-compleo-teal" />,
        title: "Community Accessibility", 
        description: "Strategically located centres for easy patient access"
      },
      {
        icon: <CheckCircle className="h-6 w-6 text-compleo-teal" />,
        title: "Streamlined Pathways",
        description: "Efficient diagnostic workflows and patient journeys"
      },
      {
        icon: <Clock className="h-6 w-6 text-compleo-teal" />,
        title: "Reduced Wait Times",
        description: "Faster access to diagnostic services for patients"
      },
      {
        icon: <Users className="h-6 w-6 text-compleo-teal" />,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="animate-fade-in-up">
        ...
        <section ref={heroRef} className="section-padding bg-compleo-deep-teal text-white">
          <div className="max-w-7xl mx-auto container-padding text-center">
            ...
            <div className="flex flex-row gap-3 sm:gap-6 justify-center">
              <Link href={heroContent.button1.href} onClick={() => trackCTAClick(heroContent.button1.analytics, '/services/community-diagnostic-centres')}>
                <Button 
                  size="lg" 
              ...
              <Link href={heroContent.button2.href} onClick={() => trackCTAClick(heroContent.button2.analytics, '/services/community-diagnostic-centres')}>
                <Button 
                  size="lg" 
        ...
        <PillCTA
          heading={pageData?.ctaSection?.title || "Partner with Us for Community Health"}
          description={pageData?.ctaSection?.description || "Let's work together to establish diagnostic centres that serve your community's healthcare needs effectively."}
          primaryButton={{ 
