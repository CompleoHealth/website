      ...
      <main id="main-content" className="animate-fade-in-up">
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div className={`transition-all duration-700 ${heroInView ? 'animate-slide-in-left opacity-100' : 'opacity-0 translate-x-[-50px]'}`}>
                <h1 className="heading-1 mb-4">
                  {heroContent.title} <span className="text-compleo-yellow">{heroContent.highlightWord}</span>
                </h1>
                <p className="body-large text-gray-200 mb-6 leading-relaxed">{heroContent.subtitle}</p>
                <div className="flex flex-row gap-3 sm:gap-6">
                  <Link href={heroContent.primaryButton.href} onClick={() => trackCTAClick(heroContent.primaryButton.trackingLabel, '/about')}>
                    <Button 
                      size="lg" 
                    ...
                    >
                      <div className="flex flex-col items-center gap-1 sm:gap-1.5">
                        <div className="bg-white/20 rounded-full p-1">
                          <heroContent.primaryButton.icon className="h-4 w-4 sm:h-5 sm:w-5 group-hover:pulse transition-transform duration-300" />
                        </div>
                        <div className="flex flex-col items-center">
                          <span className="text-sm sm:text-base font-semibold tracking-wider">{heroContent.primaryButton.text}</span>
                          <span className="text-xs font-medium opacity-90">{heroContent.primaryButton.subtext}</span>
                        </div>
                      </div>
                  ...
                  <Link href={heroContent.secondaryButton.href} onClick={() => trackCTAClick(heroContent.secondaryButton.trackingLabel, '/about')}>
                    <Button 
                      size="lg" 
                    ...
                    >
                      <div className="flex flex-col items-center gap-1 sm:gap-1.5">
                        <div className="bg-compleo-deep-teal/20 rounded-full p-1">
                          <heroContent.secondaryButton.icon className="h-4 w-4 sm:h-5 sm:w-5 group-hover:pulse transition-transform duration-300" />
                        </div>
                        <div className="flex flex-col items-center">
                          <span className="text-sm sm:text-base font-semibold tracking-wider">{heroContent.secondaryButton.text}</span>
                          <span className="text-xs font-medium opacity-90">{heroContent.secondaryButton.subtext}</span>
                        </div>
                      </div>
                    </Button>
                  </Link>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className={`max-w-4xl mx-auto transition-all duration-700 ${storyInView ? 'animate-fade-in-up opacity-100' : 'opacity-0 translate-y-8'}`}>
              <div className="text-center mb-16">
                <h2 className="heading-2 text-compleo-deep-teal mb-6">{companyOverview.title}</h2>
              ...
              <div className="bg-gradient-to-br from-white to-gray-50/30 rounded-2xl shadow-2xl p-8 lg:p-12 border-2 border-gray-100 hover:shadow-3xl transition-shadow duration-300">
                ...
                <div className="mt-10 pt-8 border-t border-gray-200">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                    {companyOverview.features.map((feature, index) => (
                      <div key={index} className="space-y-4">
                        <div className="flex justify-center mb-4">
                          {feature.iconType === 'medical' ? (
                            <MedicalIcon icon={feature.icon} />
                          ) : (
                            <BrandedIcon icon={feature.icon} variant={feature.iconType as any} />
                          )}
                        </div>
        ...
        <section className="section-padding bg-white">
          <div className="max-w-7xl mx-auto container-padding">
            {/* Header */}
            <div className="text-center mb-16">
              <h2 className="heading-2 text-compleo-deep-teal mb-6">{valuesSection.title}</h2>
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
                <div className="flex items-center justify-start h-full">
                  <div className="max-w-2xl text-white text-left">
                    <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">{ctaContent.title}</h2>
                    <p className="text-xl text-gray-300 mb-8">{ctaContent.description}</p>
                    <div className="flex justify-start">
                      <Link href={ctaContent.buttonHref}>
